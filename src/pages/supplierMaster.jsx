...
/* ============================================================================
   供应商账户管理 —— 1:1 复刻真实 SAAS「供应商 > 账户管理」(#/accountManagement)
   本模块为复刻，非进销存新增，故不加红框标注
   ============================================================================ */
const ACCOUNTS = [
  { no: "SN00000021", name: "JOJO供应商", phone: "18100010002", subject: "公司", accType: "分账到账",
    walletId: "-", walletName: "-", merchantNo: "123", merchantKey: "****", merchantSecret: "****", status: "已启用" },
];
...
export function SupplierAccount() {
  ...
  const [toast, tip] = useToast();
  const setStatus = (no, v) => setRows((rs) => rs.map((r) => (r.no === no ? { ...r, status: v } : r)));

  return (
    <>
      <div className="filters">
        <div className="row">
          <div className="field"><label>时间查询</label>
            <div className="date">
              <input className="ctl w-sm" placeholder="开始时间" readOnly />
              <span className="sep">-</span>
              <input className="ctl w-sm" placeholder="结束时间" readOnly />
            </div>
          </div>
          <div className="field"><label>状态</label>
            <select className="ctl w-sm" defaultValue=""><option value="">请选择</option><option>已启用</option><option>已禁用</option></select>
          </div>
        ...
        <table className="tbl-tight">
          <thead>
            <tr>
              ...
              <th className="tw">供应商编号</th><th className="tw">供应商名称</th><th className="tw">联系电话</th>
              <th className="tw">主体类型</th><th className="tw">账户类型</th><th className="tw">钱包ID</th>
              <th className="tw">钱包个人名称</th><th className="tw">商户号</th><th className="tw">商户Key</th>
              <th className="tw">商户秘钥</th><th className="tw">状态</th><th style={{ minWidth: 76 }}>操作</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.no}>
                ...
                <td className="tw">{r.name}</td>
                <td className="tw mono">{r.phone}</td>
                <td className="tw">{r.subject}</td>
                <td className="tw">{r.accType}</td>
                <td className="tw mono">{r.walletId}</td>
                <td className="tw">{r.walletName}</td>
                <td className="tw mono">{r.merchantNo}</td>
                <td className="tw mono">{r.merchantKey}</td>
                <td className="tw mono">{r.merchantSecret}</td>
                <td className="tw"><span className={`tag ${r.status === "已启用" ? "" : "gray"}`}>{r.status}</span></td>
                <td className="tw">
                  <div className="op-col">
                    <button className="gray" onClick={() => setToggle(r)}>{r.status === "已启用" ? "禁用" : "启用"}</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {toggle && (
        <Confirm
          title={toggle.status === "已启用" ? "确认禁用账户" : "确认启用账户"}
          text={toggle.status === "已启用"
            ? `禁用后「${toggle.name}」的分账资金将无法到账。是否继续？`
            : `启用后「${toggle.name}」可正常接收分账资金。是否继续？`}
          okText={toggle.status === "已启用" ? "确认禁用" : "确认启用"}
          onOk={() => {
            setStatus(toggle.no, toggle.status === "已启用" ? "已禁用" : "已启用");
            tip(`「${toggle.name}」账户已${toggle.status === "已启用" ? "禁用" : "启用"}`);
            setToggle(null);
          }}
          onCancel={() => setToggle(null)}
        />
